...
check_cuda_availability() {
    ...
    if command -v nvcc &> /dev/null; then
        print_info "CUDA compiler found - enabling full NVIDIA CUDA support"
        CUDA_AVAILABLE=true
        NVIDIA_FLAGS="--enable-cuda-nvcc --enable-libnpp"
    else
        print_warning "CUDA compiler not found - using NVIDIA headers only (runtime detection)"
        CUDA_AVAILABLE=false
        NVIDIA_FLAGS=""
    fi
}
...
install_dependencies() {
    ...
    # Try to install CUDA if available, but don't fail if not
    if ! command -v nvcc &> /dev/null; then
        print_info "Attempting to install NVIDIA CUDA toolkit..."
        if sudo apt-get install -y nvidia-cuda-toolkit 2>/dev/null; then
            print_info "CUDA toolkit installed successfully"
        else
            print_warning "CUDA toolkit installation failed or not available"
            print_warning "NVIDIA support will use headers only (runtime detection)"
        fi
    fi
...
# Function to build nvenc support (headers only - works without CUDA toolkit)
build_nvenc() {
    print_info "Setting up NVIDIA encoder/decoder headers..."
    print_info "Note: CUDA toolkit not required - FFmpeg will detect NVIDIA drivers at runtime"
    cd "${SRC_DIR}"
    ...
    print_info "NVIDIA headers installed (runtime detection will work with or without CUDA toolkit)."
}
...
build_ffmpeg() {
    ...
    CONFIG_OPTIONS=(
        ...
        # NVIDIA support (always enable headers - runtime detection)
        "--enable-nvenc"
        "--enable-nvdec"
...
verify_build() {
    ...
    "${BUILD_DIR}/bin/ffmpeg" -encoders | grep nvenc || print_warning "No NVENC encoders found (normal without NVIDIA GPU)"
    ...
    if [ "$CUDA_AVAILABLE" = true ]; then
        print_info "CUDA toolkit detected: Full NVIDIA support enabled"
    else
        print_info "CUDA toolkit not found: NVIDIA support via runtime detection only"
    fi
...
main() {
    ...
    # Re-check CUDA after installation attempt
    check_cuda_availability
    ...
    # NVIDIA headers (works with or without CUDA toolkit)
    build_nvenc
    ...
    if [ "$CUDA_AVAILABLE" = true ]; then
        print_info "NVIDIA CUDA: Full toolkit support enabled"
    else
        print_info "NVIDIA CUDA: Runtime detection only (no toolkit found)"
    fi
    print_info "======================================="
