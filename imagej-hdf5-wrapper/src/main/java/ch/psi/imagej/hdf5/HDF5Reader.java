...
public class HDF5Reader implements PlugIn {
	 ...
	 * arg is a space separated list of arguments that can be passed to the run
	 * method.
	...
	public ImageStack open(String arg, boolean interactive, String filename, String nameOfDataset,
	...
	public ImageStack open(String arg, boolean interactive, String filename, String nameOfDataset, boolean virtualstack,
		...
		try {
			...
			// TODO to be removed - Workaround virtual stack - keep HDF5 file open at the
			// end
			...
			for (Dataset var : selectedDatasets.getDatasets()) {
				...
				logger.info("Reading dataset: " + datasetName + " Dimensions: " + numberOfDimensions + " Type: "
				...
				if (numberOfDimensions == 5 && dimensions[4] == 3) {
					...
					if (selectedDatasets.isVirtualStack()) {
						...
						if (selectedDatasets.getSlice() != null) {
							...
							for (int indexToRead = 0; indexToRead < dimensions[0]; indexToRead = indexToRead
									+ selectedDatasets.getModulo()) {
							...
							// Select what to readout
							long[] selected = var.getSelectedDims();
							long totalSelectedSpacePoints = 1;
							selected[1] = dimensions[1];
							selected[2] = dimensions[2];

							for (int i = 0; i < var.getRank(); i++) {
								totalSelectedSpacePoints *= (long) dimensions[i];
							}
							...
							if (totalSelectedSpacePoints > Integer.MAX_VALUE) {
								...
								// loading by chunks
								for (int i = 0; i < (int) dimensions[0] / zChunk; ++i) {
									start[0] = i * zChunk;
									selected[0] = zChunk;
									wholeDataset = var.read();
									for (int lev = 0; lev < zChunk; ++lev) {
										int startIdx = lev * size;
										addSlice(stack, wholeDataset, startIdx, size);
									}
								}

								if (((int) dimensions[0] % zChunk) > 0) {
									start[0] = (int) dimensions[0] / zChunk * zChunk;
									selected[0] = ((int) dimensions[0] % zChunk);
									wholeDataset = var.read();
									for (int lev = 0; lev < selected[0]; ++lev) {
										int startIdx = lev * size;
										addSlice(stack, wholeDataset, startIdx, size);
									}
								}
	 ...
	 * 
	 * @param datasets
	 * @return List of datasets to visualize. If nothing selected the list will be
	 *         empty
